...
class TestScript(canonicalresource.CanonicalResource):
...
class TestScriptScope(backboneelement.BackboneElement):
    ...
    conformance: fhirtypes.CodeableConceptType = Field(
        ...
        description=(
            ...
            "considered conformant with the artifact: required - all tests must "
            "pass, optional - all test are expected to pass but non-pass status may"
            " be allowed."
        ),
        # if property is element of this resource.
        element_property=True,
    ...
    @classmethod
    def elements_sequence(cls):
        """returning all elements names from ``TestScriptScope`` according specification,
        with preserving original sequence order.
        """
        return [
            "id",
